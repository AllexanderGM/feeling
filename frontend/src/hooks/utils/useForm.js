...
const useForm = (totalSteps, formData) => {
  ...
  const validateCurrentStep = useCallback(
    (showDebug = false) => {
      if (showDebug) {
        Logger.debug(Logger.CATEGORIES.UI, `Validating step ${currentStep}`, { currentStep, formData })
      }
      ...
      if (!validation.isValid) {
        ...
        Logger.warn(Logger.CATEGORIES.UI, 'Validation errors found', { errors: validation.errors })
      } else {
        Logger.debug(Logger.CATEGORIES.UI, 'Validation successful for step', { currentStep })
      }
