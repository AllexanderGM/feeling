...
import { getAllTours, getTourById, createTour, updateTour, deleteTour } from '@services'
...
const useTour = () => {
  ...
  const filterToursByCategory = useCallback(
    category => {
      if (!category) return tours

      return tours.filter(tour => tour.tags?.some(tag => tag?.toLowerCase() === category.toLowerCase()))
    },
    [tours]
  )
