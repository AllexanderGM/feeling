...
const ActiveConversationsSection = ({ conversations, searchTerm, onSelectChat, getCategoryIcon }) => {
  ...
  const handleArchiveConversation = (e, convId) => {
    e.stopPropagation()
    Logger.info('Archivando conversación', Logger.CATEGORIES.USER, { conversationId: convId })
  }

  const handleMuteConversation = (e, convId) => {
    e.stopPropagation()
    Logger.info('Silenciando conversación', Logger.CATEGORIES.USER, { conversationId: convId })
  }

  const handleDeleteConversation = (e, convId) => {
    e.stopPropagation()
    Logger.warn('Eliminando conversación', Logger.CATEGORIES.USER, { conversationId: convId })
  }

  const handlePinConversation = (e, convId) => {
    e.stopPropagation()
    Logger.info('Fijando conversación', Logger.CATEGORIES.USER, { conversationId: convId })
  }
      ...
      <div className='space-y-3'>
        {filteredConversations.map(conversation => (
          <Card
            ...
            <CardBody className='p-4' onClick={() => onSelectChat(conversation)}>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className='flex items-center gap-2 mb-1'>
                        {conversation.isTyping ? (
                          ...
                          <p className={`text-sm truncate ${conversation.unreadCount > 0 ? 'text-gray-200 font-medium' : 'text-gray-400'}`}>
                      ...
                      <div className='flex items-center gap-2 text-xs text-gray-500'>
                        <span>
                          {conversation.location} • {conversation.distance} km
                        </span>
                      </div>
                    </div>
        ...
        <Card className='bg-gray-700/20 border-gray-600/20'>
          <CardBody className='p-4'>
            <div className='grid grid-cols-2 md:grid-cols-4 gap-4 text-center'>
              <div>
                <div className='text-lg font-bold text-blue-400'>{filteredConversations.length}</div>
                <div className='text-xs text-gray-400'>Total activas</div>
              </div>
              <div>
                <div className='text-lg font-bold text-green-400'>{filteredConversations.filter(c => c.unreadCount > 0).length}</div>
                <div className='text-xs text-gray-400'>Con mensajes nuevos</div>
              </div>
              <div>
                <div className='text-lg font-bold text-yellow-400'>{filteredConversations.filter(c => c.isOnline).length}</div>
                <div className='text-xs text-gray-400'>En línea ahora</div>
              </div>
              <div>
                <div className='text-lg font-bold text-purple-400'>{filteredConversations.filter(c => c.isTyping).length}</div>
