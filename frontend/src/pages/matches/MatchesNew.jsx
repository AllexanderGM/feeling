    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockData.suggestedUsers.map((user) => (
          <Card key={user.id} className="bg-gray-800/40 backdrop-blur-sm border-gray-700/50 overflow-hidden hover:scale-105 transition-transform">
            <CardBody className="p-0">
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-gray-100">{user.name}, {user.age}</h3>
                    <p className="text-sm text-gray-400">{user.location} • {user.distance}km</p>
                  </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <Button
                    ...
                    variant="bordered"
                    isIconOnly
                    onPress={() => handleAddToFavorites(user)}
                  >
        ...
        <Card key={match.id} className="bg-gray-800/40 backdrop-blur-sm border-gray-700/50">
          <CardBody className="p-4">
            <div className="flex items-center gap-4">
              <img
                src={match.targetUser.images[0]}
                alt={match.targetUser.name}
                className="w-16 h-16 rounded-lg object-cover"
              />
              <div className="flex-1">
                <h3 className="font-bold text-gray-100">{match.targetUser.name}, {match.targetUser.age}</h3>
                <p className="text-sm text-gray-400">
                  Enviado {new Date(match.sentAt).toLocaleDateString()}
                </p>
              </div>
      ...
      {mockData.receivedMatches.map((match) => (
        <Card key={match.id} className="bg-gray-800/40 backdrop-blur-sm border-gray-700/50">
          <CardBody className="p-4">
            <div className="flex items-center gap-4">
              <img
                src={match.initiatorUser.images[0]}
                alt={match.initiatorUser.name}
                className="w-16 h-16 rounded-lg object-cover"
              />
              <div className="flex-1">
                <h3 className="font-bold text-gray-100">{match.initiatorUser.name}, {match.initiatorUser.age}</h3>
                <p className="text-sm text-gray-400">
                  Recibido {new Date(match.receivedAt).toLocaleDateString()}
                </p>
              </div>
              <div className="flex items-center gap-2">
      ...
      {mockData.acceptedMatches.map((match) => (
        <Card key={match.id} className="bg-gray-800/40 backdrop-blur-sm border-gray-700/50">
          <CardBody className="p-4">
            <div className="flex items-center gap-4">
              <img
                src={match.otherUser.images[0]}
                alt={match.otherUser.name}
                className="w-16 h-16 rounded-lg object-cover"
              />
              <div className="flex-1">
                <h3 className="font-bold text-gray-100">{match.otherUser.name}, {match.otherUser.age}</h3>
                <p className="text-sm text-gray-400">
                  Match desde {new Date(match.acceptedAt).toLocaleDateString()}
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                {match.contactUnlocked && (
                  <Button
                    ...
                    onPress={() => handleViewContact(match)}
                  >
                    Contacto
                  </Button>
                )}
        ...
        <Card key={user.id} className="bg-gray-800/40 backdrop-blur-sm border-gray-700/50">
          <CardBody className="p-4">
            <div className="flex items-center gap-3">
              ...
              />
              <div className="flex-1">
                <h3 className="font-medium text-gray-100">{user.name}, {user.age}</h3>
