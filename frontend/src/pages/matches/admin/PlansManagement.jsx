...
const PlansManagement = memo(() => {
  ...
  const handleCreatePlan = useCallback(
    async planData => {
      try {
        ...
        // Agregar sortOrder si no existe
        const planDataWithDefaults = {
          ...planData,
          sortOrder: planData.sortOrder || 0
        }

        await matchService.createPlan(planDataWithDefaults)
    ...
    [handleError, handleSuccess, loadPlansData]
  )
  ...
  const handleEditPlan = useCallback(
    async planData => {
      try {
        setLoading(true)
        Logger.info('Editing plan', { planData, planId: selectedPlan.id }, { category: Logger.CATEGORIES.SERVICE })

        // Agregar sortOrder si no existe
        const planDataWithDefaults = {
          ...planData,
          sortOrder: planData.sortOrder || selectedPlan.sortOrder || 0
        }

        await matchService.updatePlan(selectedPlan.id, planDataWithDefaults)
    ...
    [selectedPlan, handleError, handleSuccess, loadPlansData]
  )
  ...
  const handleDeletePlan = useCallback(async () => {
    try {
      ...
      await matchService.deletePlan(selectedPlan.id)
    ...
    <>
      ...
      <div className='w-full space-y-6'>
        ...
        <div className='space-y-4'>
          ...
          <TablePagination
            filteredItemsLength={sortedPlans.length}
            page={page}
            pages={pages}
            selectedKeys={selectedKeys}
            onNextPage={() => setPage(prev => Math.min(prev + 1, pages))}
