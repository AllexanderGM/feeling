...
const MatchHistorySection = ({ history, searchTerm, getCategoryIcon }) => {
  ...
  const handleRestoreConversation = match => {
    Logger.info('Restaurar conversación', { matchName: match.name }, { category: Logger.CATEGORIES.UI })
  }

  const handleDeleteForever = match => {
    Logger.info('Eliminar permanentemente match', { matchName: match.name }, { category: Logger.CATEGORIES.UI })
  }

  const handleArchiveMatch = match => {
    Logger.info('Archivar match', { matchName: match.name }, { category: Logger.CATEGORIES.UI })
  }
