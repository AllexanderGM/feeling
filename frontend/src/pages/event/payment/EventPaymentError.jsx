...
import { XCircle, AlertTriangle, RefreshCw, ArrowLeft, HelpCircle, MessageCircle, Calendar } from 'lucide-react'
import { APP_PATHS } from '@constants/paths'
import LiteContainer from '@components/layout/LiteContainer.jsx'

const EventPaymentError = () => {
  ...
  const handleRetryPayment = () => {
    if (event) {
      navigate(APP_PATHS.USER.EVENT_CHECKOUT.replace(':eventId', event.id), { state: { event } })
    } else {
  ...
  return (
    <>
      <Helmet>
        <title>Error en la Reserva - Feeling</title>
        <meta content='Hubo un problema al procesar tu reserva' name='description' />
      </Helmet>

      <LiteContainer ariaLabel='Página de error de reserva' className='gap-6 max-w-4xl !pt-0 !min-h-0 py-8'>
        {/* Header */}
              ...
              <div className='space-y-3'>
                ...
                {event && (
                  <div className='bg-gray-700/30 rounded-lg p-4'>
                    <div className='text-sm text-gray-400 mb-1'>Evento que intentabas reservar:</div>
                    <div className='text-base font-semibold text-gray-100'>{event.title}</div>
                    {event.price > 0 && (
...
export default EventPaymentError
