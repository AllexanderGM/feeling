...
const UnifiedEventTable = memo(({
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <p className="text-sm font-medium text-default-800">
                {event.destination?.city || 'N/A'}
              </p>
              <p className="text-xs text-default-500">
                {event.destination?.country || 'N/A'}
              </p>
            </div>
          </div>
        )
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <p className="text-sm font-semibold text-default-800">
                ${(event.adultPrice || 0).toLocaleString()}
              </p>
              {event.childPrice && (
                <p className="text-xs text-default-500">
                  Niños: ${event.childPrice.toLocaleString()}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1 max-w-xs">
            {event.tags?.slice(0, 2).map((tag, index) => (
              <Chip
                ...
                size="sm"
                variant="flat"
                color={EVENT_CATEGORY_COLORS[tag] || 'default'}
                className="text-xs"
              >
                {tag}
              </Chip>
      ...
      case 'availability':
        return (
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <p className="text-sm font-medium text-default-800">
                {event.availability?.[0]?.availableSlots || 0} plazas
              </p>
              <p className="text-xs text-default-500">
                {event.availability?.[0]?.availableDate || 'Sin fecha'}
              </p>
            </div>
          </div>
        )
  ...
  const headerColumns = visibleColumns === 'all' 
    ? columns 
    : columns.filter(column => visibleColumnsArray.includes(column.uid))
