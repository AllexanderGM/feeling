...
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, Card, CardBody, Avatar, Chip } from '@heroui/react'
...
const DeleteEventModal = memo(({ isOpen, onClose, onSuccess, eventData }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      Logger.error(Logger.CATEGORIES.SERVICE, 'delete_event', 'Error deleting event', { error, eventId: eventData?.id })
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      <ModalContent>
        <ModalHeader className='flex flex-col gap-1'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <h2 className='text-xl font-semibold text-danger-600'>Eliminar Evento</h2>
              <p className='text-sm text-default-500 font-normal'>Esta acción no se puede deshacer</p>
            </div>
          </div>
        </ModalHeader>
        ...
        <ModalBody className='gap-6'>
            ...
            <CardBody className='p-4'>
              <div className='flex items-start gap-4'>
                ...
                <div className='flex-1 space-y-3'>
                  <div>
                    <h3 className='text-lg font-semibold text-default-800'>{eventData.name}</h3>
                    <p className='text-sm text-default-600 line-clamp-2'>{eventData.description}</p>
                  </div>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <span className='text-sm text-default-700'>${(eventData.adultPrice || 0).toLocaleString()}</span>
                    </div>

                    {/* Disponibilidad */}
                    <div className='flex items-center gap-2'>
                      <Calendar className='w-4 h-4 text-default-400' />
                      <span className='text-sm text-default-700'>{eventData.availability?.[0]?.availableSlots || 0} plazas</span>
                    </div>
          ...
          <div className='bg-warning-50 border border-warning-200 rounded-lg p-4'>
            <h4 className='font-semibold text-warning-800 mb-2'>Consecuencias de eliminar este evento:</h4>
