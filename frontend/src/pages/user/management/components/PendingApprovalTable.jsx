...
import { Check, X, Eye, UserIcon, AlertCircle } from 'lucide-react'
...
const PendingApprovalTable = memo(({ pendingUsers, onApprove, onReject, loading }) => {
        ...
        case 'name':
          ...
          return (
            <div className='flex items-center gap-3'>
              {hasImage ? (
                <Avatar radius='lg' src={hasImage} alt={`${user.name || 'Usuario'}`} className='w-10 h-10' />
              ) : (
  ...
  return (
    <>
      <Table aria-label='Tabla de usuarios pendientes de aprobación' className='min-h-[400px]' removeWrapper>
              ...
              <div className='space-y-4'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                  <div>
                    ...
                    <p className='text-sm text-gray-400'>{calculateAge(selectedUser.birthDate || selectedUser.dateOfBirth)} años</p>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <p className='text-sm text-gray-400 bg-gray-800 p-3 rounded-lg'>{selectedUser.description || 'Sin descripción'}</p>
                </div>
                ...
                <div className='flex items-center gap-2 p-3 bg-yellow-900/20 rounded-lg'>
                  ...
                  <p className='text-sm text-yellow-300'>
                    Registrado hace{' '}
                    {Math.floor((new Date() - new Date(selectedUser.registeredAt || selectedUser.createdAt)) / (1000 * 60 * 60 * 24))} días
                  </p>
                </div>
              </div>
            )}
