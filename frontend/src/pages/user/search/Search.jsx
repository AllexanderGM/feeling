...
import { Card, CardBody, Avatar, Button, Chip, Input, Slider, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Switch, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure } from '@heroui/react'
import {
...
const Search = () => {
  ...
  const profilesData = useMemo(() => [
    {
      ...
      description: 'Amante de la música y los viajes. Siempre buscando nuevas aventuras y experiencias auténticas. Me encanta conectar con personas genuinas.',
  ...
  const filteredProfiles = useMemo(() => {
    ...
    if (!selectedCategories.includes('all')) {
      filtered = filtered.filter(profile => 
        selectedCategories.includes(profile.category.toLowerCase())
      )
    }
  ...
  const handlePass = (profileId) => {
    console.log('Pasar perfil:', profileId)
    setIsAnimating(true)
    setTimeout(() => {
      nextProfile()
      setIsAnimating(false)
  ...
  const handleSuperLike = (profileId) => {
    console.log('Super like a perfil:', profileId)
    setIsAnimating(true)
    setTimeout(() => {
      nextProfile()
      setIsAnimating(false)
  ...
  const handleSendMessage = (profile) => {
    console.log('Enviar mensaje a:', profile.name)
  }
          ...
          {filteredProfiles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">
                Intenta ajustar los filtros para encontrar más personas
              </p>
              <Button
                variant="bordered"
            ...
            <>
                ...
                {selectedProfile && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400">{selectedProfile.age} años • {selectedProfile.location}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <Chip color="danger" variant="flat" size="sm">
                          {selectedProfile.compatibility}% match
                        </Chip>
                        <Chip 
                          color={selectedProfile.isOnline ? "success" : "default"}
                          variant="flat"
                          size="sm"
                        >
                      ...
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-300">{selectedProfile.location} ({selectedProfile.distance} km)</span>
                          </div>
                          <div className="flex items-center gap-2">
              ...
              <ModalFooter>
                <Button 
                  ...
                  startContent={favorites.has(selectedProfile?.id) ? <Heart className="w-4 h-4 fill-current" /> : <HeartOff className="w-4 h-4" />}
                  onPress={() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                ...
                <p className="text-green-200 text-xs">
                  Usa el modo Discovery para una experiencia más natural y divertida
                </p>
              </div>
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                <SearchIcon className="w-4 h-4 text-blue-400 mx-auto mb-2" />
                <p className="text-blue-200 text-xs">
                  Utiliza filtros específicos para encontrar personas con intereses similares
                </p>
              </div>
