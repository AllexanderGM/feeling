import { useMemo, useEffect, useCallback, forwardRef, useImperativeHandle, useState } from 'react'
import { useForm, Controller } from 'react-hook-form'
import { yupResolver } from '@hookform/resolvers/yup'
import { Switch, Card, CardBody, Chip, Avatar, Spinner } from '@heroui/react'
...
const StepConfiguration = forwardRef(({ onStepComplete, overrideUser = null, saveOptions = {} }, ref) => {
  const { user } = useAuth()
  const activeUser = overrideUser || user
  const [isSaving, setIsSaving] = useState(false)
  const defaultValues = useMemo(() => getDefaultValuesForStep(4, activeUser), [activeUser])
  ...
  useEffect(() => {
    ...
    reset(getDefaultValuesForStep(4, activeUser), { keepDefaultValues: false })
  }, [activeUser, reset])
  ...
  const userLastName = getUserLastName(activeUser)
  const userEmail = getUserEmail(activeUser)
  const userPhone = getUserPhone(activeUser)
  const userPhoneCode = getUserPhoneCode(activeUser)
  const userCountry = getUserCountry(activeUser)
  const userCity = getUserCity(activeUser)
  const userLocality = getUserLocality(activeUser)
  const userAge = getUserAge(activeUser)
  const userImages = getUserImages(activeUser) || []
  const userMainImage = userImages.length > 0 ? userImages[0] : null

  // Step 2: Características
  const userTags = getUserTags(activeUser) || []
  const userGender = getUserGender(activeUser)
  const userMaritalStatus = getUserMaritalStatus(activeUser)
  const userEducation = getUserEducation(activeUser)
  const userProfession = getUserProfession(activeUser)
  const userHeight = getUserHeight(activeUser)

  // Step 3: Preferencias
  const userInterest = getUserCategoryInterest(activeUser)
