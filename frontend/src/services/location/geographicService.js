...
class GeographicService extends ServiceREST {
  ...
  async getAllGeographicData(forceRefresh = false) {
    ...
    const context = 'obtener datos geográficos completos'

    try {
      this.isLoadingAll = true
      const result = await ServiceREST.get(API_ENDPOINTS.GEOGRAPHIC.ALL)
  ...
  async getCitiesByCountry(countryName) {
    ...
    try {
      const url = API_ENDPOINTS.GEOGRAPHIC.CITIES_BY_COUNTRY.replace('{countryName}', encodeURIComponent(countryName))
  ...
  async getLocalitiesByCity(cityName) {
    ...
    try {
      const url = API_ENDPOINTS.GEOGRAPHIC.LOCALITIES_BY_CITY.replace('{cityName}', encodeURIComponent(cityName))
      const result = await ServiceREST.get(url)
  ...
  async preloadPopularCountries(popularCountries = ['Colombia', 'México', 'España', 'Argentina']) {
    const promises = popularCountries.map(async country => {
      try {
        ...
        this.Logger.warn(`Error precargando país`, { country, error: error.message })
      }
    })
  ...
  logError(operation, error) {
    ...
    this.Logger.serviceError(operation, error, 'geographicService')
  }
}
