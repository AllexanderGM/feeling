...
public class SelfModificationAuthorizationFilter extends OncePerRequestFilter {
    ...
    // Patrones de rutas que requieren autorización de auto-modificación
    private static final Pattern[] SELF_MODIFICATION_PATTERNS = {
        Pattern.compile("^/users/([^/]+)/?$"), // PUT /users/{email}
        Pattern.compile("^/users/([^/]+)/.*$"), // PUT /users/{email}/anything
        Pattern.compile("^/api/support/my-complaints/([^/]+)/?$") // Acceso a quejas específicas
    };

    // Métodos que requieren verificación
    private static final String[] RESTRICTED_METHODS = {"PUT", "PATCH", "DELETE"};
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            // Solo aplicar el filtro a métodos que modifican datos
            ...
            String targetIdentifier = extractTargetIdentifier(requestURI);
    ...
    /**
     * Verifica si el método HTTP requiere verificación de autorización
     */
    private boolean isRestrictedMethod(String method) {
        for (String restrictedMethod : RESTRICTED_METHODS) {
            if (restrictedMethod.equals(method)) {
                return true;
            }
        }
        return false;
     ...
     */
    private String extractTargetIdentifier(String requestURI) {
        for (Pattern pattern : SELF_MODIFICATION_PATTERNS) {
            Matcher matcher = pattern.matcher(requestURI);
            if (matcher.matches()) {
                return matcher.group(1); // Primer grupo capturado
            }
        }
        return null;
    }
    ...
    private boolean isUserIdMatch(String targetIdentifier, String currentUserEmail, HttpServletRequest request) {
        ...
        try {
            ...
            // No es numérico, asumir que es email
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        // No filtrar rutas administrativas (ya están protegidas por roles)
        ...
        if (requestURI.startsWith("/auth/") || 
            requestURI.startsWith("/geographic/") ||
            requestURI.startsWith("/user-attributes/") ||
            requestURI.startsWith("/category-interests/") ||
            requestURI.startsWith("/tags/")) {
            return true;
        }

        // No filtrar métodos de solo lectura
